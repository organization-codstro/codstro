...
export const UserRecordService = {
   ...
   */
  async getUserRecord(userId: number) {
   ...
   * 참조 테이블: ai_user_records
   */
  async updateUserRecord(recordId: number, summary: string) {
    const { data, error } = await supabase
      .from("ai_user_records")
      .update({
        ai_user_record_summary: summary,
        ai_user_record_created_date: new Date().toISOString(),
      })
      .eq("ai_user_record_id", recordId)
      .select()
      .single();
...
export const AiSummaryService = {
  async generateUserSummary(userId: number, recentMessages: string[]) {
    const prompt = `
