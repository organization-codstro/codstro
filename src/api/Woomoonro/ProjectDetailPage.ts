...
import { CloneCodingsResponse, ProjectTodoResponse, UserCloneCodingsResponse } from "../../types/api/Woomoonro/ProjectDetailPage";
...
export const CloneCodingService = {
   ...
   */
  async getProjectDetail(projectId: number): Promise<CloneCodingsResponse | null> {
  ...
  async generateProjectGuide(
    ...
    const prompt = `프로젝트 "${projectTitle}"을(를) ${techStack.join(
      ", "
    )}로 구현하려고 합니다. 
