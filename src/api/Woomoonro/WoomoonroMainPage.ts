...
export const MainProjectService = {
   ...
   */
  async getAllProjectsWithUserStatus(userId: number) {
    try {
  ...
  async getFilteredProjects(
    ...
    try {
      ...
      if (params.selectedFilter === "bookmarked") {
        query = query.eq("user_status.user_clone_codings_is_bookmarked", true);
      } else if (params.selectedFilter !== "all") {
        query = query.eq(
      ...
      if (params.selectedDifficulty !== "all") {
        query = query.eq("clone_codings_difficulty", params.selectedDifficulty);
      }

      // 3. 제목 검색
      if (params.searchQuery) {
        query = query.ilike("clone_coding_title", `%${params.searchQuery}%`);
      }
      ...
      const { error } = await supabase.from("user_clone_codings").upsert(
        {
          ...
          user_clone_codings_is_bookmarked: !isBookmarked,
        },
        {
