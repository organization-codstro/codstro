 ...
 */
export const RecommendedStudyPlansService = {
  /**
   ...
   */
  async getRecommendedPlans(searchQuery: GetRecommendedPlansParams = "") {
    try {
      let query = supabase
        .from("study_plans")
        .select(
   ...
   */
  async toggleBookmark(params: ToggleBookmarkParams) {
    try {
      const { data, error } = await supabase
        .from("study_plans")
        .update({ study_plan_is_archived: !params.currentState })
   ...
   */
  async addToMyPlans(params: AddToMyPlansParams) {
    try {
      const { data: newPlan, error: planError } = await supabase
        .from("study_plans")
        .insert([
          {
            study_plan_name: params.targetPlan.study_plan_name,
            study_plan_description: params.targetPlan.study_plan_description,
            study_plan_image_url: params.targetPlan.study_plan_image_url,
