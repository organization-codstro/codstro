...
export const ProjectMeetingService = {
      ...
      .insert([
        {
          ...
          project_tasks_log_sender: params.sender,
          project_tasks_log_message: params.message,
          project_tasks_log_meeting_index: params.meetingIndex,
  ...
  async getAiResponse(
    ...
    const { data: history } = await supabase
      ...
      .eq("project_meeting_room_id", roomId)
      .order("project_tasks_log_created_at", { ascending: false })
      .limit(5);

    const context = history
      ?.reverse()
      .map(
        (m) => `${m.project_tasks_log_sender}: ${m.project_tasks_log_message}`
      )
      .join("\n");
   ...
   * @table project_meeting_rooms, project_tasks_logs, project_meeting_summarys
   */
  async deleteMeeting(roomId: string) {
    // 1. 관련 로그 삭제
    await supabase
      .from("project_tasks_logs")
      .delete()
   ...
   */
  async incrementMeetingIndex(roomId: string, currentIndex: number) {
    const { data, error } = await supabase
      .from("project_meeting_rooms")
